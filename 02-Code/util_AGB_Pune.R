...
agbSummary <- function(df, group_by = "ward", col.dbh = "dbh", col.height = "height_m") {
  out <- df %>% group_by(!!!syms(group_by)) %>%
    mutate(dbh.min = min(c_across(all_of(col.dbh)), na.rm = TRUE),
           ...
           dbh.q.75 = quantile(c_across(all_of(col.dbh)), na.rm = TRUE, probs = .75),
           ...
           H.q.75 = quantile(c_across(all_of(col.height)), na.rm = TRUE, probs = .75),
           ...
           wd.q.75 = quantile(wd.mean, na.rm = TRUE, probs = .75),
    ...
    mutate(agb.min = min(c_across(starts_with("simu.")), na.rm = TRUE),
           ...
           agb.q.75 = quantile(c_across(starts_with("simu.")), na.rm = TRUE, probs = .75),
